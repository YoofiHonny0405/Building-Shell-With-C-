...
std::string processEcho(const std::vector<std::string>& args) {
    ...
    for (size_t i = 1; i < args.size(); i++) {
        ...
        if (token.size() >= 2 && ((token.front() == '"' && token.back() == '"') ||
            ...
            if (quoteChar == '\'') {
                // Single quotes: output literally.
                if (!output.empty()) output.push_back(' ');
                output += token;
                continue;
            }
        }
        // Process escapes for token (assume double-quoted or unquoted).
        std::string processed;
        bool escape = false;
        for (size_t j = 0; j < token.size(); j++) {
            char c = token[j];
            if (escape) {
                // If the character is a double quote, add it and discard the backslash.
                if (c == '"') {
                    processed.push_back('"');
                } else if (c == '\\') {
                    processed.push_back('\\');
