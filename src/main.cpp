...
int main() {
    ...
    while (true) {
        ...
        if (command == "echo") {
            ...
            // Now, perform a replacement: for any occurrence of the sequence "\\'" in the result, replace it with "\'".
            size_t rpos = 0;
            while ((rpos = result.find("\\\\'", rpos)) != std::string::npos) {
                result.replace(rpos, 3, "\\'");
                rpos += 2; // move past the replaced sequence
            }
