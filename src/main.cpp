...
std::vector<std::string> split(const std::string &str, char delimiter) {
    ...
    for (size_t i = 0; i < str.size(); ++i) {
        ...
        if (c == '\\') {
            escapeNext = true;
            token.push_back(c); // keep the backslash for later processing
            continue;
        }
...
std::string processEcho(const std::vector<std::string>& args) {
    ...
    for (size_t i = 1; i < args.size(); i++) {
        ...
        // Check if token is wrapped in matching quotes.
        if (token.size() >= 2 && token.front() == token.back() &&
            (token.front() == '"' || token.front() == '\'')) {
            char quoteChar = token.front();
            token = token.substr(1, token.size() - 2);
            if (quoteChar == '"') {
                // Process escapes for double-quoted tokens.
            ...
            // For single-quoted tokens, leave escapes as literal.
        } else {
            // For unquoted tokens, process escapes normally.
            std::string processed;
            bool escape = false;
            for (char c : token) {
                if (escape) {
